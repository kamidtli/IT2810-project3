...
const resolvers = {
  Query: {
    ...
    findMoviesBasedOnTitle: async (root, { title, pagination, skip, sort }) =>
    ...
    findMoviesBasedOnDirector: async (
      ...
      await Movie.find({ directors: { $regex: director, $options: 'i' } })
        ...
        .limit(pagination)
        .sort(sort),
    findMoviesBasedOnYear: async (root, { year, pagination, skip, sort }) =>
    ...
    findMoviesBasedOnYearRange: async (
      ...
      await Movie.find({ year: { $lte: max, $gte: min } })
        ...
        .limit(pagination)
        .sort(sort),
    findMoviesBasedOnGenre: async (root, { genre, pagination, skip, sort }) =>
  ...
  findImdbRatingPerYear: async (root, { year }) =>
    await Movie.find({ year }).then(
      data =>
        data
          .reduce((obj, item) => {
            if (
